...
describe('augment overlay performance safeguards', () => {
  ...
  it('renders the arena floating overlay from precomputed recommendation data', async () => {
    ...
    expect(floatingOverlay).toContain("import ArenaRecommendationCard from './ArenaRecommendationCard.vue'")
    expect(floatingOverlay).toContain(':recommend-score="augment.recommendScore"')
    expect(floatingOverlay).toContain(':win-rate="augment.winRate"')
    expect(floatingOverlay).toContain(':pick-rate="augment.pickRate"')
