...
应用会在实际对局阶段自动截图并识别海克斯选择界面，然后按窗口偏好在顶部浮窗和右侧推荐列表中展示海克斯胜率和推荐度。普通游戏画面约每 1.5 秒检查一次，确认进入海克斯选择界面且识别到候选后恢复为 500 ms；自动截图和 F8 手动刷新都使用 `1024x576`。识别到三张海克斯时会优先按左、中、右卡片区域确定顺序。
...
- 按 F8 手动刷新：跳过门禁和完整 OCR 冷却，立即执行一次全尺寸识别并刷新浮窗。
- 适用于自动识别失败、切换/升级海克斯后需要重新查看时。
...
| 置信度 > 90% | 降低误报 |
| 海克斯组合变化 | 自动识别时相同组合不会重复通知；F8 手动刷新会强制重新推送当前组合 |
